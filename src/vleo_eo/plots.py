...
def plot_delivery_timeline(
    ...
    """
    Plot geographic visualization of delivery timeline for sample accesses.

    Matches Cell 24 and 26 of the original notebook.
    ...
    sample_indices : List[int], optional
        Specific access indices to plot. If None, selects randomly.
    output_path : Path, optional
    ...
    if sample_indices is None:
        # Use first mode to get indices
        first_mode = list(mode_dfs.values())[0]
        available_indices = list(first_mode.index[:min(3, len(first_mode))])
    else:
        available_indices = sample_indices

    # Create custom colormap for contact times
    colors_cmap = ['#00ff00', '#2ecc71', '#f1c40f', '#e67e22']
    custom_cmap = LinearSegmentedColormap.from_list("custom", colors_cmap, N=4)
    ...
    for idx in available_indices:
        for mode_name, df in mode_dfs.items():
            ...
            try:
                # Calculate ground track segments
                squint_start = access.get('Squint_Start_Time', access['Start_Time'])
                if pd.isna(squint_start):
                    squint_start = access['Start_Time']

                squint_start_dt = squint_start.to_pydatetime() if hasattr(squint_start, 'to_pydatetime') else squint_start

                # Ground track
                ground_track = calculate_ground_track(
                    tle_data, sat_id,
                    squint_start_dt - timedelta(minutes=2),
                    duration=timedelta(minutes=10)
                )
                if ground_track:
                    lons, lats = zip(*ground_track)
                    ax.plot(lons, lats, color='black', linewidth=1, alpha=0.5,
                            transform=ccrs.PlateCarree(), label='Ground Track')

                # Imaging segment
                dwell_time = access.get('Collect_Dwell_Time', 30)
                imaging_track = calculate_ground_track(
                    tle_data, sat_id, squint_start_dt,
                    duration=timedelta(seconds=dwell_time)
                )
                if imaging_track:
                    lons, lats = zip(*imaging_track)
                    ax.plot(lons, lats, color='green', linewidth=2,
                            transform=ccrs.PlateCarree(), label='Imaging')

                # Plot target AOI
                ax.plot(access['AOI_Lon'], access['AOI_Lat'], 'bs', markersize=8,
                        transform=ccrs.PlateCarree(), fillstyle='none', label='Target AOI')

                # Plot ground stations and comm cones
                for gs in ground_stations:
                    ax.plot(gs.lon, gs.lat, '^', color='black', markersize=6,
                            transform=ccrs.PlateCarree())
                    ...
                    valid_col = f'Valid_Contact_{gs.name}'
                    if access.get(valid_col, False):
                        try:
                            comm_cone = create_communication_cone(
                                gs.lon, gs.lat, gs.min_elevation_deg,
                                tle_data[sat_id].altitude_km * 1000
                            )
                            ax.add_geometries(
                                [comm_cone],
                                crs=ccrs.PlateCarree(),
                ...
                # Set extent
                if ground_track:
                    all_lons = [p[0] for p in ground_track]
                    all_lats = [p[1] for p in ground_track]
                    all_lons.append(access['AOI_Lon'])
                    all_lats.append(access['AOI_Lat'])
                    buffer = 5
                    ax.set_extent([
                        min(all_lons) - buffer,
                        max(all_lons) + buffer,
                ...
                # Title
                ax.set_title(
                    f"Access {idx}: {mode_name}\n"
                    f"Satellite: {sat_id}, Start: {access['Start_Time']}"
                )
                ax.legend(loc='upper right')

            except Exception as e:
                print(f"Warning: Could not plot delivery timeline for access {idx}: {e}")
                ax.set_title(f"Access {idx}: {mode_name} (Error)")

            plt.tight_layout()
            figures.append(fig)

            if output_path:
                fig.savefig(output_path / f'delivery_timeline_{mode_name}_{idx}.png',
